...
void CMenuWidget::paint()
{
	...
	int w_body = width + sb_width;
	int y_body = y + hheight;
	int y_body_end = y + height;
	...
	// paint body shadow
	frameBuffer->paintBoxRel(x+SHADOW_OFFSET, y_body+SHADOW_OFFSET, w_body, h_body, COL_MENUCONTENTDARK_PLUS_0, RADIUS_LARGE, CORNER_BOTTOM);
	// paint body background
	frameBuffer->paintBoxRel(x, y_body, w_body, h_body, COL_MENUCONTENT_PLUS_0, RADIUS_LARGE, (fbutton_count ? 0 /*CORNER_NONE*/ : CORNER_BOTTOM));

	item_start_y = y+hheight;
	paintItems();
	washidden = false;
	if (fbutton_count){
		CComponentsFooter footer(x, y_body_end, w_body, fbutton_height, 0, NULL, true);
		footer.setButtonLabels(fbutton_labels, fbutton_count, 0, (w_body/max(fbutton_count, 2)-1)-20);
...
void CMenuWidget::paintHint(int pos)
{
	...
	int rad = fbutton_height != 0 ? 0 : RADIUS_LARGE;
